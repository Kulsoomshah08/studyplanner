...
export default function Home({ onNavigate }: HomeProps) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
      {/* Hero */}
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-brand-700 via-accent-700 to-brand-900 px-6 py-10 sm:px-10 sm:py-14 text-white shadow-glow animate-fade-in">
        <div className="absolute top-0 right-0 w-72 h-72 bg-white/10 rounded-full -translate-y-24 translate-x-24 blur-2xl" />
        <div className="absolute bottom-0 left-1/3 w-48 h-48 bg-accent-400/20 rounded-full translate-y-20 blur-2xl" />
        <div className="relative">
          <p className="text-brand-100 font-medium text-sm mb-2">Welcome back!</p>
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight mb-2">
            Hey {userName}, ready to study?
          </h1>
          <p className="text-brand-100/80 text-sm mb-1">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 bg-white/10 text-white font-semibold px-5 py-2.5 rounded-xl text-sm hover:bg-white/20 transition-colors border border-white/20"
            >
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
        {statCards.map((card, i) => {
          ...
          return (
            <div
              key={card.label}
              className="bg-ink-900 rounded-2xl border border-ink-700/60 p-5 shadow-card hover:border-brand-500/30 transition-colors animate-slide-up"
            ...
            >
              ...
              <p className="text-sm text-slate-400 mt-1.5">{card.label}</p>
            </div>
          );
        })}
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
        {/* Progress card */}
        <div className="bg-ink-900 rounded-2xl border border-ink-700/60 p-6 shadow-card animate-slide-up">
          <div className="flex items-center gap-2 mb-4">
            <TrendingUp className="w-5 h-5 text-brand-400" />
            <h2 className="text-lg font-semibold text-white">Completion Progress</h2>
          </div>
        ...
        {/* Upcoming tasks */}
        <div className="lg:col-span-2 bg-ink-900 rounded-2xl border border-ink-700/60 p-6 shadow-card animate-slide-up">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <CalendarClock className="w-5 h-5 text-brand-400" />
              <h2 className="text-lg font-semibold text-white">Upcoming & Overdue</h2>
            </div>
            <button
              onClick={() => onNavigate('tasks')}
              className="text-sm font-medium text-brand-400 hover:text-brand-300 inline-flex items-center gap-1"
            >
              View all
          ...
          {error && (
            <div className="text-sm text-rose-300 bg-rose-500/10 border border-rose-500/25 rounded-lg px-4 py-3">
              {error}
            </div>
          )}
          ...
          {!error && !loading && upcomingTasks.length === 0 && (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              <div className="w-14 h-14 rounded-2xl bg-brand-500/10 flex items-center justify-center mb-3">
                <Circle className="w-7 h-7 text-brand-400/60" />
              </div>
            ...
            <ul className="space-y-2.5">
              {upcomingTasks.map((task) => {
                ...
                return (
                  <li
                    key={task.id}
                    className="flex items-center gap-3 p-3 rounded-xl border border-ink-700/50 hover:border-brand-500/30 hover:bg-brand-500/5 transition-colors"
                  >
      ...
      {/* Quick tips */}
      <section className="mt-8 bg-ink-900 rounded-2xl border border-ink-700/60 p-6 shadow-card animate-slide-up">
        <div className="flex items-center gap-2 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div
              key={tip.title}
              className="rounded-xl bg-brand-500/5 border border-brand-500/15 p-4 hover:bg-brand-500/10 transition-colors"
            >
              <p className="text-sm font-semibold text-brand-300">{tip.title}</p>
              <p className="text-sm text-slate-400 mt-1.5 leading-relaxed">{tip.body}</p>
            </div>
          ))}
        </div>
        <button
          onClick={() => onNavigate('assistant')}
          className="mt-5 inline-flex items-center gap-2 text-sm font-medium text-brand-400 hover:text-brand-300"
        >
